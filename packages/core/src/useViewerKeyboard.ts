 ...
 * Captures global keyboard events and sends them via the active HID transport.
 */
...
export function useViewerKeyboard() {
  ...
  if (!transport) {
    throw new Error('[useViewerKeyboard] HIDTransportKey not provided.')
  }
  ...
  function handleEvent(event: KeyboardEvent, pressed: boolean): void {
    ...
    if (transport.isConnected && !transport.isConnected.value) return
    ...
    const hidCode = mapCodeToHid(code)
    if (hidCode < 0) {
...
const direct: Record<string, number> = {
  ...
  NumpadDivide: 0x54, NumpadMultiply: 0x55, NumpadAdd: 0x57,
  NumpadSubtract: 0x56, NumpadEnter: 0x58, NumpadDecimal: 0x63,
  Numpad0: 0x62, Numpad1: 0x59, Numpad2: 0x5a, Numpad3: 0x5b,
  Numpad4: 0x5c, Numpad5: 0x5d, Numpad6: 0x5e, Numpad7: 0x5f,
  Numpad8: 0x60, Numpad9: 0x61, NumpadEqual: 0x67,
  ControlLeft: 0xe0, ControlRight: 0xe4, ShiftLeft: 0xe1,
  ShiftRight: 0xe5, AltLeft: 0xe2, AltRight: 0xe6,
  MetaLeft: 0xe3, MetaRight: 0xe7,
}

function mapCodeToHid(code: string): number {
...
function isWasmReady(): boolean {
  return typeof window !== 'undefined' && !!((window as any)._wasmModule)
}
