...
import HostTargetToggle from '../usb/HostTargetToggle.vue'
import PasteTextDialog from '../paste/PasteTextDialog.vue'
import SettingsModal from '../settings/SettingsModal.vue'

const { isConnected } = useSerial()
const { sendMediaKey, switchUsbToHost, switchUsbToTarget, sendKeyUp } = useSerialCommands()
...
<template>
  <div class="flex items-center gap-2 px-3 py-1.5 bg-slate-900 border-t border-slate-800 h-11 shrink-0 overflow-x-auto">
    ...
    <div class="flex items-center gap-0.5 shrink-0">
      ...
      <button @click="sendMedia(MEDIA_WAKE)" :disabled="!isConnected" class="p-1.5 rounded hover:bg-amber-500/20 disabled:opacity-30 disabled:cursor-not-allowed text-amber-400" title="Wake">
        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 3v1m0 16v1m9-9h-1M4 12H3m15.364 6.364l-.707-.707M6.343 6.343l-.707-.707m12.728 0l-.707.707M6.343 17.657l-.707.707M16 12a4 4 0 11-8 0 4 4 0 018 0z"/></svg>
      </button>
      <button @click="sendMedia(MEDIA_SLEEP)" :disabled="!isConnected" class="p-1.5 rounded hover:bg-indigo-500/20 disabled:opacity-30 disabled:cursor-not-allowed text-indigo-400" title="Sleep">
    ...
    <div class="flex items-center gap-0.5 shrink-0">
      ...
      <button @click="sendMedia(MEDIA_PLAY)" :disabled="!isConnected" class="p-1 rounded hover:bg-slate-700 disabled:opacity-30 disabled:cursor-not-allowed text-slate-400" title="Play/Pause">
        <svg class="w-3.5 h-3.5" fill="currentColor" viewBox="0 0 24 24"><path d="M8 5v14l11-7z"/></svg>
      </button>
      <button @click="sendMedia(MEDIA_NEXT)" :disabled="!isConnected" class="p-1 rounded hover:bg-slate-700 disabled:opacity-30 disabled:cursor-not-allowed text-slate-400" title="Next Track">
    ...
    <div class="flex items-center gap-0.5 shrink-0">
      ...
      <button @click="sendMedia(MEDIA_MUTE)" :disabled="!isConnected" class="p-1 rounded hover:bg-slate-700 disabled:opacity-30 disabled:cursor-not-allowed text-slate-400" title="Mute">
        <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5.586 15H4a1 1 0 01-1-1v-4a1 1 0 011-1h1.586l4.707-4.707C10.923 3.663 12 4.109 12 5v14c0 .891-1.077 1.337-1.707.707L5.586 15z" /><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 14l2-2m0 0l2-2m-2 2l-2-2m2 2l2 2"/></svg>
      </button>
      <button @click="sendMedia(MEDIA_VOL_UP)" :disabled="!isConnected" class="p-1 rounded hover:bg-slate-700 disabled:opacity-30 disabled:cursor-not-allowed text-slate-400" title="Volume Up">
    ...
    <!-- Paste Text -->
    <button @click="showPaste = true" :disabled="!isConnected" class="p-1.5 rounded hover:bg-slate-700 disabled:opacity-30 disabled:cursor-not-allowed text-slate-400" title="Paste Text to Target">
